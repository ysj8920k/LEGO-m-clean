...
def depth_calibration_raw(pipeline,number_of_frames):
    ...
    while t<=number_of_frames:
        ...
        depth_image,color_image=post_process(frameset)
        depth_colour_list.append(depth_image)
        t+=1


    calib_depth=avg_depth_colour(depth_colour_list)
    return calib_depth
"""
...
def compare_depth(x,y,calib,depth_img):
    image=depth_img
    ...
    img_val = depth_img[x-10:x+10, y-10:y+10].astype(float)
    cal_val = calib[x-10:x+10, y-10:y+10].astype(float)
    img_val=img_val
    cal_val=cal_val
    ...
    cv2.imshow('depth', image)
    cv2.waitKey(1)
    return dist_img-dist_cal,depth_img,dist_cal
...
try:
    ...
    while True:
        ...
        for t in range(0,100):
            ...
            depth_image, color_image=post_process(frameset)
            dist,Measure,cal=compare_depth(900,500,calib_depth,depth_image)
            dist_list.append(dist)
            Measure_list.append(Measure)
            cal_list.append(cal)

        print('Average Depth')
        print(np.mean(dist_list))#*(1/0.00005))
        print('Calibrated')
        print(np.mean(cal_list))
        print('Measured')
        print(np.mean(Measure_list))
