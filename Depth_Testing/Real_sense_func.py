...
def get_xyA(frames,webcam,roi,profile):
    avg_center_list_x=[]
    avg_center_list_y=[]
    avg_of_angle=[]
    list_of_x=[]
    ...
    # Load a model
    model_type = YOLO("Yolo-LEGO-test/runs/segment/train123/weights/best.pt")
...
def main_mv_belt(min_frames, webcam,profile, Rworld_cords=np.array([0, -55.107, 0, 193.119]), roi=[0, 100, 0, 100],T_inpx=[0,0,0], px_pr_mm = 0, Calibrating=True):
    if Calibrating:
        ...
        if len(result_x) == 2:
            ...
            px_length = calc_length(
                [min(result_x), max(result_x)], [min(result_y), max(result_y)])
            mm_length = calc_length(
                [Rworld_cords[0], Rworld_cords[1]], [Rworld_cords[2], Rworld_cords[3]])

            px_pr_mm = px_length / mm_length
            roi = [min(result_x)-(50*px_pr_mm), max(result_x)+(50*px_pr_mm), min(result_y)+(30*px_pr_mm), max(result_y)-(30*px_pr_mm)]
            T_inpx = [min(result_x), min(result_y),
                      sum(result_angle) / len(result_angle)]

            print('The region of interest is:  ' + str(roi))
            print('PX to mm ratio:  ' + str(px_pr_mm))
            print('Camera 0 is offset, by:  ' +
                  str(T_inpx) + ' in relation to Robot Frame')
            to_continue()
        else:
...
if __name__ == '__main__':
    pipeline,profile= import_realsense()
    main_mv_belt(1000,pipeline,profile,Calibrating=False)
    pipeline.stop()