...
# Function to separate lists based on their length
def separate_lists(nested_list):
    list_5_elements = []
    list_3_elements = []

    def _separate(sub_list):
        for item in sub_list:
            if isinstance(item, list):
                # Check if the item is a list of non-list items
                if all(isinstance(i, list) for i in item):
                    _separate(item)
                else:
                    if len(item) == 5:
                        list_5_elements.append(item)
                    elif len(item) == 3:
                        list_3_elements.append(item)
            else:
                continue
    
    _separate(nested_list)
    return list_5_elements, list_3_elements
...
def process_image(depth_image,depth_image_raw ,img,depth_scale,model_type,model_colour,class_names_type,class_names_model_colour,colors,roi,calib_depth):
        ...
        for seg, box in zip(masks_type.data.cpu().numpy(), boxes_type):
            ...
            if Angle_temp!=None:
                ...
                if float(box.conf)>=0.3:
                    ...
                    type_results.append([center[0],center[1],Angle_temp,Brick_type,dist])
    ...
    if boxes_colour is not None:
        ...
        for box in boxes_colour:
            ...
            if float(box.conf)>=0.5:
                ...
                colors_results.append([center[0],center[1],colour_type])
    ...
    return [[type_results],[colors_results]]    
...
def get_xyA(frames,webcam,roi,profile, calib_depth):
    ...
    # Load a model
    model_type = YOLO("Yolo-LEGO-test/runs/segment/train167/weights/best.pt")
    model_colour = YOLO("Yolo-LEGO-test/runs/detect/train27/weights/best.pt")
    ...
    for i in range(0,frames):
        ...
        framesets.append((depth_image,depth_image_raw ,img,depth_scale,model_type,model_colour,class_names_type,class_names_model_colour,colors,roi,calib_depth))

    num_processes = len(framesets)
    with Pool(processes=num_processes) as pool:
        results = pool.starmap(process_image, framesets)
    ...
    type_results, colour_results = separate_lists(results)
    ...
    for result in type_results:
        ...
        for i in range(0,len(avg_center_list_x[:])):
            ...
            if x+50 > avg_center_list_x[i] >x-50 and y+50 > avg_center_list_y[i] >y-50:
                brick_index=i
                #print('Found existing')
                New_brick=False
                list_of_depth[i].append(dist)

                list_of_angle[i].append(Angle)
        ...
        if New_brick==True:
            ...
            avg_of_angle.append([Angle])
            avg_center_list_x.append([x])
            avg_center_list_y.append([y])
...
if __name__ == '__main__':
    ...
    main_mv_belt(10,pipeline,profile,calib_depth,Calibrating=True)
    ...
    print(f"All image processing tasks have completed in {total_time:.2f} seconds.")