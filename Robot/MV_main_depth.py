                ...
                for seg, box in zip(masks_type.data.cpu().numpy(), boxes_type):
                    ...
                    if Angle_temp!=None:
                        ...
                        if float(box.conf)>=0.3:
                            ...
                            #### SAVE in brick or create new mk9o0m9o0 
                            for i in range(0,len(avg_center_list_x[:])):

                                if center[0]+10 > avg_center_list_x[i] >center[0]-10 and center[1]+10 > avg_center_list_y[i] >center[1]-10:
                                    brick_index=i
...
def main_mv_belt(min_frames, webcam,profile, calib_depth, Rworld_cords=np.array([-4, -139.885,  0, 194.822]), roi=[0, 100, 0, 100],T_inpx=[0,0,0], px_pr_mm = 0, Calibrating=True):
    ...
    std_threshold_angle = 0.6  # Example threshold for std_angle

    for index, row in results.iterrows():
        depth_criteria_1 = (-155 >= row['Depth']) & (row['Depth'] >= -160.5)
        depth_criteria_2 = (-162 > row['Depth']) & (row['Depth'] >= -167)
